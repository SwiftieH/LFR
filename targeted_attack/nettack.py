...
import torch
from targeted_attack.base_attack import BaseAttack
...
class Nettack(BaseAttack):
    ...
    def attack(self, features, adj, labels, target_node, n_perturbations, direct=True, n_influencers= 0, ll_cutoff=0.004, verbose=True, **kwargs):
        ...
        self.adj_no_selfloops = self.ori_adj.copy()
        ...
        if verbose:
            ...
            if attack_structure and attack_features:
                ...
                print("##### Attack node with ID {} only using feature perturbations #####".format(target_node))
            elif attack_structure:
                print("##### Attack node with ID {} only using structure perturbations #####".format(target_node))
            if direct:
        ...
        if len(self.influencer_nodes) == 0:
            if not direct:
                ...
                #infls = self.get_attacker_nodes(n_influencers)
                #self.influencer_nodes = infls.astype("int")
    ...
    def get_attacker_nodes(self, n=5, add_additional_nodes = False):
        ...
        neighbors = self.adj_no_selfloops[self.target_node].nonzero()[1]
        ...
        if len(neighbors) >= n:  # do we have enough neighbors for the number of desired influencers?
            ...
            if len(neighbors) == 1 and neighbors[0] == self.target_node:
                neighbors = np.array([])
